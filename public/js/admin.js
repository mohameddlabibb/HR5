// public/js/admin.js
// Client-side JavaScript for the Somabay Handbook Admin Panel.
// Handles admin login, page management (CRUD), sidebar reordering, image uploads, and design options.
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchPagesList() {
        try {
            ...
            if (error.message.includes('401')) {
                showMessage(loginMessage, 'Session expired. Please log in again.', true);
    ...
    addPageForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                alert(data.message || 'Failed to add page/chapter.');
            }
        } catch (error) {
            console.error('Error adding page:', error);
            alert('An error occurred while adding the page/chapter.');
        }
    });
    ...
    addMenuForm.addEventListener('submit', async (e) => {
        ...
        if (!newMenuName) {
            showMessage(addMenuMessage, 'Menu name is required.', true);
            return;
        }
        ...
        try {
            ...
            if (response.ok) {
                showMessage(addMenuMessage, 'Menu created successfully!', false);
                $('#addMenuModal').modal('hide');
                addMenuForm.reset();
                loadMenuBuilder();
            } else {
                showMessage(addMenuMessage, data.message || 'Failed to create menu.', true);
            }
        } catch (error) {
            console.error('Error adding menu:', error);
            showMessage(addMenuMessage, 'An error occurred while creating the menu.', true);
        }
    });
    ...
    addWidgetForm.addEventListener('submit', async (e) => {
        ...
        if (!name || !type) {
            showMessage(addWidgetMessage, 'Widget name and type are required.', true);
            return;
        }
        ...
        try {
            ...
            if (response.ok) {
                showMessage(addWidgetMessage, 'Widget created successfully!', false);
                ...
                showMessage(addWidgetMessage, data.message || 'Failed to create widget.', true);
            }
        } catch (error) {
            console.error('Error adding widget:', error);
            showMessage(addWidgetMessage, 'An error occurred while creating the widget.', true);
        }
    });
    ...
    imageUploadForm.addEventListener('submit', async (e) => {
        ...
        if (fileInput.files.length === 0) {
            showMessage(uploadMessage, 'Please select a file to upload.', true);
            return;
        }
        ...
        try {
            ...
            if (response.ok) {
                showMessage(uploadMessage, `Image uploaded successfully! URL: ${data.file_path}`, false);
                fileInput.value = ''; // Clear file input
                // Optionally, update an image field in an open modal
                // e.g., document.getElementById('edit-page-image').value = data.file_path;
            } else {
                showMessage(uploadMessage, data.message || 'Image upload failed.', true);
            }
        } catch (error) {
            console.error('Error uploading image:', error);
            showMessage(uploadMessage, 'An error occurred during image upload.', true);
        }
    });
